<template>
  <div class="min-h-screen flex flex-col items-center justify-center bg-beige-200">
    <div class="bg-white p-12 rounded-lg shadow-lg w-full max-w-md">
      <div v-if="loading" class="text-center text-base opacity-80">Loading...</div>
      <div v-else>
        <div v-if="!tokenValid" class="text-center">
          <div class="flex items-center justify-center mb-8">
            <svg
              width="48"
              height="48"
              viewBox="0 0 48 48"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M14 22V14C14 11.3478 15.0536 8.8043 16.9289 6.92893C18.8043 5.05357 21.3478 4 24 4C26.6522 4 29.1957 5.05357 31.0711 6.92893C32.9464 8.8043 34 11.3478 34 14V22M10 22H38C40.2091 22 42 23.7909 42 26V40C42 42.2091 40.2091 44 38 44H10C7.79086 44 6 42.2091 6 40V26C6 23.7909 7.79086 22 10 22Z"
                stroke="#1E1E1E"
                stroke-width="4"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </svg>
          </div>

          <div class="text-2xl md:text-3xl font-semibold leading-tight mb-6">Access Denied</div>
          <div class="text-base leading-relaxed mb-4">
            Please check your access token and try again.
          </div>
          <div v-if="error" class="text-red-600 text-base">{{ error }}</div>
        </div>
        <div v-else>
          <div class="text-2xl md:text-3xl font-semibold leading-tight mb-8 text-center">
            Authenticate
          </div>
          <div class="mb-8 text-center">
            <div class="text-base font-semibold text-gray-600 mb-3">Access Token</div>
            <div class="font-mono text-sm bg-gray-100 p-3 rounded text-black break-all">
              {{ token }}
            </div>
          </div>
          <form @submit.prevent="handleSubmit" class="flex flex-col gap-6">
            <div class="flex flex-col">
              <input
                placeholder="Enter your email"
                v-model="email"
                type="email"
                @input="clearError"
                :class="[
                  'p-4 bg-gray-100 rounded-lg border focus:outline-none focus:ring-2 text-base',
                  error
                    ? 'border-red-400 focus:ring-red-500'
                    : 'border-gray-300 focus:ring-blue-500',
                ]"
                required
              />
              <div v-if="error" class="text-red-600 text-sm mt-2 p-2 bg-red-50 rounded">
                {{ error }}
              </div>
            </div>
            <button
              type="submit"
              class="px-6 py-4 bg-black text-white font-semibold hover:bg-gray-700 transition duration-300 disabled:opacity-50 rounded-lg text-base"
            >
              Continue
            </button>
          </form>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { authenticateApplicationSession, verifyApplicationToken } from '@/api/applications'
import router from '@/router'
import { onMounted, ref } from 'vue'
import { useRoute } from 'vue-router'

const route = useRoute()
const token = (route.params.token as string) || ''

const email = ref('')
const error = ref('')
const tokenValid = ref(false)
const loading = ref(true)

function clearError() {
  error.value = ''
}

async function handleSubmit() {
  if (!email.value.trim()) {
    error.value = 'Email is required'
    return
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value.trim())) {
    error.value = 'Please enter a valid email address'
    return
  }
  try {
    const resp = await authenticateApplicationSession(token, email.value.trim())
    console.log('Authentication successful:', resp)
    console.log(JSON.stringify(resp.session))
    router.push({
      name: 'FillApplicationSession',
      params: { id: resp.session.id },
      state: { session: JSON.stringify(resp.session) },
    })
  } catch (err: any) {
    error.value = err.message || 'Authentication failed'
  }
}

onMounted(async () => {
  loading.value = true
  try {
    const resp = await verifyApplicationToken(token)
    tokenValid.value = resp.exists
    if (!tokenValid.value) {
      error.value = 'Application does not exist or has expired.'
    }
  } catch (err) {
    error.value = 'Application does not exist or has expired.'
    tokenValid.value = false
  } finally {
    loading.value = false
  }
})
</script>

<style scoped></style>
